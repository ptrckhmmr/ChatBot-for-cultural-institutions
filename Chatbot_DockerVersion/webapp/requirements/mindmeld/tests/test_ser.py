...
def test_duration(query, predicted_texts, predicted_values):
    ...
    response_values_normalized_seconds = [r['value']['normalized']['value'] for r in responses if
                                          r['value'].get('normalized')]
...
def test_number(query, predicted_texts, predicted_values):
    ...
    response_values = [r['value']['value'] for r in responses if
                       r['value'].get('value') and r['dim'] == 'number']
...
@pytest.mark.parametrize(
    ...
    [
        ("fifth",
         ["fifth"],
         [5]),
        ("call the eighth contact",
         ["eighth"],
         [8]),
        ("second option",
         ["second"],
         [2]),
        ("call the 2nd contact",
         ["2nd"],
         [2]),
    ]
)
...
@pytest.mark.parametrize(
    "query, predicted_texts, predicted_values",
    [
        ("place a call to +1649198338",
         ["+1649198338"],
         ["(+16) 49198338"]),
        ("please call 4388995284",
         ["4388995284"],
         ["4388995284"]),
        ("dial 772 771 21 14",
         ["772 771 21 14"],
...
@pytest.mark.parametrize(
    "query, predicted_texts, predicted_values",
    [
        ("make it 5 degrees cooler in the bedroom",
         ["5"],
         [5]),
        ("turn it down a few degrees",
         ["a few degrees"],
         [3]),
        ("please lower temperature by 10",
         ["10"],
         [10]),
...
def test_temperature(query, predicted_texts, predicted_values):
    ...
    response_values = [r['value']['value'] for r in responses if
                       r['value'].get('value') and r['dim'] == 'temperature']
...
@pytest.mark.parametrize(
    ...
    [
        ...
        ("Set an alarm for 615",
         ["615"],
         ["2018-12-13T06:15:00.000-08:00"]),
    ]
)
def test_time(query, predicted_texts, predicted_values):
    ...
    response_values = [r['value']['value'] for r in responses if
                       r['value'].get('value') and r['dim'] == 'time']
...
def test_system_entity_recognizer_component_no_config(kwik_e_mart_app_path):
    ...
    result = system_entity_recognizer.SystemEntityRecognizer.get_instance(
        app_path=kwik_e_mart_app_path).get_response({'text': 'test'})
    assert result[1] == 200
...
def test_system_entity_recognizer_component_empty_config(food_ordering_app_path):
    ...
    result = system_entity_recognizer.SystemEntityRecognizer.get_instance(
        app_path=food_ordering_app_path).get_response({'text': 'test'})
